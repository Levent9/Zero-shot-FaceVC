...
class Decoder(nn.Module):
    ...
    def forward_kd(self, x):
        ...
        x1 = F.relu(self.convolutions[0][x])
        x2 = F.relu(self.convolutions[1][x1])
        x3 = F.relu(self.convolutions[2][x2])
        x4 = x3.transpose(1, 2)

        outputs, _ = self.lstm2(x4)
        decoder_output = self.linear_projection(outputs)

        return decoder_output, outputs, x3, x2, x1, x
...
class Decoder_ac(nn.Module):
    ...
    def forward_pseudo(self, z, lf0_embs, spk_embs, mel_target=None, if_no_grad=True):
        z = F.interpolate(z.transpose(1, 2), scale_factor=2) # (bs, 140/2, 64) -> (bs, 64, 140/2) -> (bs, 64, 140)
        z = z.transpose(1, 2) # (bs, 64, 140) -> (bs, 140, 64)
        spk_embs_exp = spk_embs.unsqueeze(1).expand(-1,z.shape[1],-1)
        lf0_embs = lf0_embs[:,:z.shape[1],:]
    ...
    def forward_kd(self, z, lf0_embs, spk_embs, mel_target=None):
        z = F.interpolate(z.transpose(1, 2), scale_factor=2) # (bs, 140/2, 64) -> (bs, 64, 140/2) -> (bs, 64, 140)
        z = z.transpose(1, 2) # (bs, 64, 140) -> (bs, 140, 64)
        spk_embs_exp = spk_embs.unsqueeze(1).expand(-1,z.shape[1],-1)
        lf0_embs = lf0_embs[:,:z.shape[1],:]
        ...
        mel_outputs, lstm_output, x3, x2, x1, x = self.decoder.forward_kd(x)
        ...
        if mel_target is None:
            ...
            if self.use_l1_loss:
                loss = loss + F.l1_loss(mel_outputs, mel_target) + \
                    F.l1_loss(mel_outputs_postnet, mel_target)
            return loss, mel_outputs_postnet, mel_outputs, lstm_output, x3, x2, x1, x 
