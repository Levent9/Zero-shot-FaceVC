...
class SpeakerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.output_layer(out) # [batch_size, fea_dim]
...
class VQEmbeddingEMA(nn.Module):
    ...
    def forward(self, x):
        ...
        x_flat = x.detach().reshape(-1, D) # [bs * 80, 64]

        distances = torch.addmm(torch.sum(self.embedding ** 2, dim=1) +
                                torch.sum(x_flat ** 2, dim=1, keepdim=True),
                                x_flat, self.embedding.t(),
                                alpha=-2.0, beta=1.0) # calculate the distance between each ele in embedding and x
        # distances.shape [bs * 80, 512]
        indices = torch.argmin(distances.float(), dim=-1)  # get minimal values index of 512 shape
...
if __name__ == '__main__':
    encoder = Encoder(80, 512, 512, 64, 256)
    input_ = torch.randn(256, 80, 128)
